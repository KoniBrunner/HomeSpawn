...
public class Updater {
	...
	public boolean shouldUpdate(String localVersion, String remoteVersion) {
		String[] locals = localVersion.replaceAll("v", "").split("\\.");
		String[] remots = remoteVersion.replaceAll("v", "").split("\\.");
		if (locals.length == remots.length && remots.length == 3) {
			int loc = Integer.parseInt(locals[0]) * 10000 + Integer.parseInt(locals[1]) * 100
					+ Integer.parseInt(locals[2]);
			int rem = Integer.parseInt(remots[0]) * 10000 + Integer.parseInt(remots[1]) * 100
					+ Integer.parseInt(remots[2]);
			return loc < rem;
		}
		return false;
	}
	...
	private boolean read() {
		try {
			...
			this.plugin.getLogger().warning("Latest available version: " + this.versionName);
